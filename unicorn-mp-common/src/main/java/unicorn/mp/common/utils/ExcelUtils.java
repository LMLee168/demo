...
public class ExcelUtils {
     ...
     */
    public static <T> List<T> readExcelData(MultipartFile excel, Class<T> rowModel, Integer headRowNumber, String endsWith) {
        ExcelListener excelListener = new ExcelListener();
        ExcelReaderBuilder readerBuilder = getReader(excel, excelListener, endsWith);
     ...
     * @return Excel 数据 list
     */
    public static <T> List<T> excelImport(MultipartFile excel, ExcelService excelBaseService, Class rowModel, String endsWith) {
     ...
     * @return Excel 数据 list
     */
    public static <T> List<T> excelImport(MultipartFile excel, ExcelService excelBaseService, Class rowModel, int sheetNo,
                                          Integer headLineNum, String endsWith) {
        ExcelListener excelListener = new ExcelListener(excelBaseService);
        ExcelReaderBuilder readerBuilder = getReader(excel, excelListener, endsWith);
        if (readerBuilder == null) {
            return null;
    ...
    private static ExcelReaderBuilder getReader(MultipartFile excel,
                                                ExcelListener excelListener, String endsWith) {
        String filename = excel.getOriginalFilename();
        if (filename == null || (!filename.toLowerCase().endsWith(endsWith) && !filename.toLowerCase().endsWith(".xlsx"))) {
            throw new StandardException("文件格式错误！");
        }
